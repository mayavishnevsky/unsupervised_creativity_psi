...
| File | Responsibility |
| --- | --- |
| [`src/creativity.py`](src/creativity.py#L1) | Prompt sampling, sigma schedule, fixed noise, equations 14/16/21, reference preparation, and the registered `creativity` reward. |
...
3. Before processing any candidate, generate one fixed reference cloud. Every
   candidate prompt in the dataset is excluded from reference sampling at
   [`main.py:59`](main.py#L59).
4. For each candidate prompt, encode its conditions and sample raw FLUX noise.
5. Run iterative pCNL MCMC to obtain reward-aware initial particles.
6. Run iterative SMC to denoise and continue reward-guided particle updates.
7. Decode and save the selected Psi image.
8. When baseline comparison is enabled, run vanilla FLUX from the preserved
...
([`config/creativity.yaml:82`](config/creativity.yaml#L82)):
...
[`config/creativity.yaml:82`](config/creativity.yaml#L82). The balanced sampler
...
- The same 48-reference statistics are used by every candidate and every
  pCNL/SMC reward evaluation in that invocation.
...
([`src/creativity.py:55`](src/creativity.py#L55)).
...
at [`src/creativity.py:474`](src/creativity.py#L474). Each reference endpoint
uses:
...
at [`src/creativity.py:498`](src/creativity.py#L498). With unchanged source
...
- The prompt is encoded independently
  ([`src/creativity.py:491`](src/creativity.py#L491)).
- Vanilla FLUX.1-schnell generates one packed latent endpoint
  ([`src/creativity.py:507`](src/creativity.py#L507)).
- The default uses four denoising steps and one image per prompt
  ([`config/creativity.yaml:87`](config/creativity.yaml#L87)).
- Reference generation and feature calculation are under
  `@torch.no_grad()` ([`src/creativity.py:460`](src/creativity.py#L460)).
- The transformer is explicitly put in evaluation mode and frozen
  ([`src/creativity.py:472`](src/creativity.py#L472)).
...
One noise tensor is sampled per integration level. The table shape is:

```text
(num_steps, 1, *latent_shape)
```

The singleton endpoint dimension is intentional
([`src/creativity.py:147`](src/creativity.py#L147)). At a given level, every
reference and every candidate uses the same Gaussian noise. Different levels
use different noise.

The table is created once from the first reference endpoint shape and
`noise_seed + seed`
([`src/creativity.py:521`](src/creativity.py#L521)). It is then reused:

- for all remaining references;
- for all candidate prompts;
- for every pCNL iteration;
- for every SMC step.

There is exactly **one noise table**, not eight tables and not one table per
prompt. `level_batch_size` controls how many levels are assembled together in
`iem_features`; it does not create additional tables or change the formula.
...
See [`src/creativity.py:248`](src/creativity.py#L248).
...
[`src/creativity.py:261`](src/creativity.py#L261) and
[`src/creativity.py:265`](src/creativity.py#L265).
...
2. **IEM probe denoiser.** It must use native FLUX linear time because equation
   ...
   ([`src/creativity.py:603`](src/creativity.py#L603)); reference probes call
   `predict_conditioned` directly
   ([`src/creativity.py:536`](src/creativity.py#L536)). Neither path calls the
...
[`src/creativity.py:267`](src/creativity.py#L267).
...
- Noise construction, equation 14 residuals, and equation 16 weights use
  float32 ([`src/creativity.py:252`](src/creativity.py#L252)).
- Reference sums and squared norms use float64 for numerical stability
  ([`src/creativity.py:290`](src/creativity.py#L290)).
- `mu_omega` is stored in float32; scalar `v_omega` remains float64
  ([`src/creativity.py:335`](src/creativity.py#L335)).
- The reward returned to Psi is float32
  ([`src/creativity.py:613`](src/creativity.py#L613)).

## Equation 21: reference statistics and reward

Let the reference features be `Phi(x'_1), ..., Phi(x'_M)`. The default is
`M=48`; the current smoke run uses `M=3`.
...
See [`src/creativity.py:276`](src/creativity.py#L276). It then computes:
...
See [`src/creativity.py:318`](src/creativity.py#L318).
...
`clamp_min(0)` at [`src/creativity.py:336`](src/creativity.py#L336) only guards
...
See [`src/creativity.py:342`](src/creativity.py#L342).
...
recomputing 48 pairwise differences on every reward call. The fixed
`mu_omega` and `v_omega` summarize the full reference cloud exactly for this
...
verified in
[`tests/test_creativity.py:228`](tests/test_creativity.py#L228).
...
([`src/flux_pipeline.py:570`](src/flux_pipeline.py#L570)).
...
The 48 references do **not** multiply the 520 candidate calls. They were
already reduced to `mu_omega` and `v_omega` once.

### One-time full reference cost

With defaults and batch size one:
...
The optional matched baseline adds four vanilla FLUX denoising steps.
...
`(levels, endpoints, ...)`
([`src/creativity.py:171`](src/creativity.py#L171)). The batch-aligned native
velocity helper is
...
See [`src/creativity.py:471`](src/creativity.py#L471).
...
([`src/creativity.py:603`](src/creativity.py#L603)).
...
| `level_batch_size` | 1 | Levels assembled per `iem_features` chunk. |
...
| `noise_seed` | 90,000,000 | Base seed for the one fixed IEM noise table. |
...
- image rewards still receive decoded tensors
  ([`tests/test_creativity.py:294`](tests/test_creativity.py#L294));
- latent rewards receive Tweedie latents before detached decode
  ([`tests/test_creativity.py:325`](tests/test_creativity.py#L325));
...
- shared noise is deterministic
  ([`tests/test_creativity.py:114`](tests/test_creativity.py#L114));
- balanced exclusions are enforced
  ([`tests/test_creativity.py:152`](tests/test_creativity.py#L152)).
...
- epoch refreshes;
- priority sampling;
- multiple noise-table assignments;
- distributed reference-statistic merging;
- training checkpoint state for IEM references.

Psi instead creates one fixed reference cloud and one fixed noise table per
invocation. Also, this repository uses frozen **FLUX.1-schnell**, not SD3.5
Medium and not the RAM CFG-distillation baseline LoRA. The IEM equations remain
applicable because FLUX uses the same linear rectified-flow velocity
parameterization required by `f=x_t-t*v`.
...
- `level_batch_size` can change batching and memory use, but not the selected
  sigmas, noise table, feature definition, or equation-21 result.
